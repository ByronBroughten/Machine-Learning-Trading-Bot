...
def get_analysis_df(analysis):
    metrics = ['test_hsl_vs_long', 'test_sl_vs_long']
    df = analysis.dataframe()
    df.sort_values(metrics[0], inplace=True, ascending=False)

    cnf_cols = [col for col in df.columns if col[:6] == 'config']
    cols_no_cnf = {col: col[7:] for col in cnf_cols}
    df = df[metrics + cnf_cols]

    df.rename(columns = cols_no_cnf, inplace = True)

    # column_mergers = [['horizon', 'horizon'], ['b_size', 'batch_size'], ['drop', 'dropout'], ['chans', 'tcn_channel_size']]
    # for cm in column_mergers:
    #     df[cm[0]] = df[cm].sum(1)
    #     df = df.drop(cm[1:], 1)
    
    to_drop = ['context_mode', 'context_out_size_arr', 'step_mins', 'tcn_output_size']
